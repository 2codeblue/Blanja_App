...
const handleChangePrimary = (id) => {
  console.log(id);    //buat kirim ke be
}
  return (
    <main
        ...
        <div className={`${styles.profilebox} my-5 ms-5 p-3 bg-white`}>
          ...
          {addressPrimary.map((item, index) => {
            ...
            (
              ...
              <div 
              className={`text-primary py-3 px-2 w-25 text-center bg-primary text-white rounded-3 ${styles.btnHover}`}
              onClick={()=>handleChangePrimary(item.id)}>Change Primary Address</div>
            </div>
            )
          )})}
        </div>
